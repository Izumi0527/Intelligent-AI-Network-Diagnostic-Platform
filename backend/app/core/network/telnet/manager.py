...
from app.core.network.base import DeviceType
...
class TelnetManager:
    ...
    def _create_connection(
        self, device_type: str, host: str, port: int, username: str, password: str
    ) -> TelnetConnection:
        """根据设备类型创建相应的连接"""
        try:
    ...
    async def connect(
        ...
        try:
            ...
            connection = self._create_connection(
                device_type, host, port, username, password
            )
            ...
            if success:
                ...
                return (
                    True,
                    f"连接成功，会话ID: {connection.session_id}",
    ...
    async def cleanup_idle_sessions(self, idle_timeout: int = 7200) -> int:
        ...
        for session_id, connection in self.sessions.items():
            if (
                connection.last_activity_time
                and current_time - connection.last_activity_time > idle_timeout
            ) or not connection.is_alive():
    ...
    async def _cleanup_session(self, session_id: str):
        ...
        if session_id in self.sessions:
            ...
            try:
                ...
                logger.debug(f"清理会话 {session_id} 断开连接失败: {str(e)}")
            del self.sessions[session_id]
